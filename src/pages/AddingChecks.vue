...
const isFormValid = computed(() => {
  return foodname.value.length > 0 && rules.number(foodcost.value) === true && rules.positive(foodcost.value) === true && payerPerson.value !== null && selectedEaters.value.length > 0;
});
...
const addProduct = () => {
  if (isFormValid.value) {
    ...
    const newProduct = {
      ...
      eatBy: peopleStore.people.filter((person) => selectedEaters.value.includes(person.id)),
      show: false,
    };
      ...
      <v-container>
        <v-list class="d-flex flex-column align-start">
          <v-list-item v-for="(product, index) in productStore.products" :key="product.id">
            <v-card>
              <v-list-item class="d-flex flex-column justify-start align-start">
                <v-container>
                  <v-card>
                    <p>Название: {{ product.foodname }}</p>
                  </v-card>
                  <v-card>
                    <p>Цена: {{ product.foodcost }}</p>
                  </v-card>
                  <v-card>
                ...
                <v-expand-transition>
                  <v-card v-if="product.show">
                    <ul>
                      <li v-for="person in product.eatBy" :key="person.id">
                        {{ person.name }}
                      </li>
                    </ul>
                  </v-card>
